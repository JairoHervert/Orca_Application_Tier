...
#include <string>
#include <stdexcept>
#include <optional>  // ← necesario para std::optional y std::nullopt

// Repo de cripto, storage, usuarios DB
...
public:
   ...
   std::string execute(const std::string &leaderEmail,
                       const std::string &leaderPassword,
                       const std::string &seniorEmail,
                       const std::string &repoName,
                       const std::string &projectAlias) {
     
      int leaderIdForCommit = -1; // para registrar commits incluso si falla algo
      ...
      try {
         ...
         if (leaderOpt.has_value()) {
            leaderIdForCommit = leaderOpt->idUser;
         }
         ...
         const std::string fullAlias = repoName + "_" + projectAlias;
         if (DBProjectRepository.existsRepoAlias(fullAlias))
            throw std::runtime_error(
               "The repository alias " + fullAlias +
               " for the repository " + repoName +
               " already exists in the database. Choose another alias."
            );
         ...
         std::string aesKeyCifradaRSA_Leader =
            cryptoRepo_.cipher_RSA_OAEP(aesKeyB64, leaderUser.publicKeyRSA.c_str());

         // 15. Cifrar la clave AES con la clave pública RSA del usuario senior, aun no la guarda en DB
         std::string aesKeyCifradaRSA_Senior =
            cryptoRepo_.cipher_RSA_OAEP(aesKeyB64, seniorOpt->publicKeyRSA.c_str());
         ...
         if (!tarDeleted)
            throw std::runtime_error("Error deleting the original tar file: " + tarPath.string());


         // 19. Verificar que el cifrado fue correcto
         if (!cifradoOk)
            throw std::runtime_error("Error ciphering the repository tar file: " + tarPath.string());


         // 20. Si el cifrado fue correcto, guardar las claves cifradas en la tabla repo_protect
         bool passwordStored_Leader =
            DBProjectRepository.addPassword_repo_user(
               leaderUser.idUser,
               repo.idProject,
               aesKeyCifradaRSA_Leader,
               fullAlias
            );
         ...
         bool passwordStored_Senior =
            DBProjectRepository.addPassword_repo_user(
               seniorOpt->idUser,
               repo.idProject,
               aesKeyCifradaRSA_Senior,
               fullAlias
            );
         ...
         // 21. Registrar commit de éxito
         DBProjectRepository.addCommit(
            leaderIdForCommit,
            std::nullopt,   // idFile (no aplica a un repo completo)
            std::nullopt,   // firma (no estás firmando aquí)
            true,           // isAccepted
            "CIPHER_REPOSITORY",
            "Repository " + repoName +
               " ciphered with alias " + fullAlias +
               " by leader " + leaderEmail +
               " and senior " + seniorEmail
         );

         // 22. Retornar la clave AES cifrada con RSA del líder para mostrar al cliente
         return aesKeyCifradaRSA_Leader;

      } catch (const std::exception &e) {
         // Registrar commit de fallo (best effort)
         try {
            DBProjectRepository.addCommit(
               leaderIdForCommit,
               std::nullopt,
               ...
               "CIPHER_REPOSITORY",
               std::string("Failed to cipher repository ") + repoName +
                  " with alias " + repoName + "_" + projectAlias +
                  " requested by leader " + leaderEmail +
                  " and senior " + seniorEmail +
                  ": " + e.what()
            );
         } catch (...) {
            // no permitir que un error en logging o BD oculte el error original
         }
